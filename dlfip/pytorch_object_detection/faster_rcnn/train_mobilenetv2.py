...
def main():
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    for param in quantized_model.backbone.parameters():
        param.requires_grad = False

    # define optimizer
    params = [p for p in quantized_model.parameters() if p.requires_grad]
    ...
    for epoch in range(init_epochs):
        ...
        mean_loss, lr = utils.train_one_epoch(quantized_model, optimizer, train_data_loader,
        ...
        coco_info = utils.evaluate(quantized_model, val_data_loader, device=device)
    ...
    torch.save(quantized_model.state_dict(), "./save_weights/pretrain.pth")
    ...
    # 冻结backbone部分底层权重
    for name, parameter in quantized_model.backbone.named_parameters():
    ...
    optimizer = torch.optim.SGD(params, lr=0.005,
                                momentum=0.9, weight_decay=0.0005)
    # learning rate scheduler: 每3 epoch将学习率降低到0.33x
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/mobile-quantized_model-{}.pth".format(epoch))
