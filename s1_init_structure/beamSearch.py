...
def calc_batch(model: torch.nn.Module, device: str, batch_x: list)->np.array:
    group_nums = []
    expanded_batch_x = []
    max_group_size = 0
    ...
    output = model(expanded_batch_x, task_id_batch)
...
def mtg_beam_search(task_num, mtg_model, device: str, beam_width: int):
