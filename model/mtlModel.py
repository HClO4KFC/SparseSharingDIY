...
class MTL_model(torch.nn.Module):
    def __init__(self, backbone_name:str, member:list, out_features:list, prune_names:list, cv_tasks_args):
        ...
        for i in member:
            ...
            mask = [torch.nn.Parameter(torch.ones(named_param.size()).to(named_param).bool(), requires_grad=False)
                    for named_param in self.backbone.parameters()]
    ...
    def forward(self, x, task_id:int):
        ...
        with torch.no_grad():
            ...
            for param_no in range(len(params)):
                ...
                mask = self.masks[ingroup_no][param_no]
                masked_param = param * mask.float()
                masked_params[param_no] = masked_param
